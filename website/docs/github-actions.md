...
The `stackql-deploy` GitHub Action will download the latest `stackql-deploy` binary from [GitHub Releases](https://github.com/stackql-labs/stackql-deploy-rs/releases).  The action invokes a `stackql-deploy` command with `inputs` (detailed below).  Here is a basic example of using the `stackql-deploy` GitHub Action in a workflow.
