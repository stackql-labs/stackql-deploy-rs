...
impl FromStr for QueryType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.trim().to_lowercase().as_str() {
            ...
            _ => Err(QueryError::InvalidType(format!("Unknown query type: {}", s))),
        }
    }
}
                    ...
                    if option_parts.len() == 2 {
                        ...
                        if let Ok(value) = option_value.parse::<u32>() {
                            match option_name {
                                ...
                                "postdelete_retry_delay" => current_options.postdelete_retry_delay = value,
...
mod tests {
    ...
    fn test_query_type_from_str() {
        ...
        assert_eq!(QueryType::from_str("createorupdate").unwrap(), QueryType::CreateOrUpdate);
        assert_eq!(QueryType::from_str("statecheck").unwrap(), QueryType::StateCheck);
    ...
    fn test_get_queries_as_map() {
        let mut queries = HashMap::new();
        queries.insert(QueryType::Exists, Query {
            query_type: QueryType::Exists,
            sql: "SELECT COUNT(*) FROM table".to_string(),
            options: QueryOptions::default(),
        });
        queries.insert(QueryType::Create, Query {
        ...
        assert_eq!(map.len(), 2);
        assert_eq!(map.get(&QueryType::Exists).unwrap(), "SELECT COUNT(*) FROM table");
        assert_eq!(map.get(&QueryType::Create).unwrap(), "INSERT INTO table VALUES (1)");
    }
}